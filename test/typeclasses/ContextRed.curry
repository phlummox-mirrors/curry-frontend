...
testD1 :: (E a, C a, G a, F a) => a -> a
testD1 x = fun2 (testD2 x)

testD2 :: (E a, C a, G a, F a) => a -> a
...
testF1 :: (E a, F a, G a, C a) => a -> a
testF1 x = fun2 (testF2 x)

testF2 :: (E a, F a, G a, C a) => a -> a
...
testL2 :: (J a, Ord a, Show a) => a -> a
testL2 x = fun4 (testL1 x)

testM1 :: (J a, C a, E a) => a -> a
...
testR1 :: (G a, F a, C a) => a -> b
testR1 x = testR2 (fun2 x)

testR2 :: (G a, F a, C a) => a -> b
testR2 x = testR1 x



testS1 :: (G a, F a) => a -> b -> c
testS1 x y = testS2 y x

testS2 :: (G b, F b) => a -> b -> c
testS2 x y = testS1 y x

testT1 :: (G a, G b, F a, F b) => a -> b -> c
testT1 x y = testT2 x y

testT2 :: (G a, G b, F a, F b) => a -> b -> c
...
testU1 :: (F a, G a, F b, G b) => a -> b -> c
testU1 x y = testU2 x y

testU2 :: (F a, G a, F b, G b) => a -> b -> c
testU2 x y = testU1 y x
  where testU1_1 :: (F a, G a) => a -> b -> c
        testU1_1 x y = testU2_1 y x

        testU2_1 :: (F b, G b) => a -> b -> c
        testU2_1 x y = testU1_1 y x
          where testU1_2 :: C a => a -> a
...
testX2 :: (F a, G b, C a) => a -> b -> Bool
testX2 x y = testX1 x y 



testY1 :: (F a, F b, G a, G b) => a -> b -> c
testY1 x y =
  let testY1_1 :: (F a, G a) => a -> b -> c
      testY1_1 x y = testY2_1 y x

      testY2_1 :: (F b, G b) => a -> b -> c
      testY2_1 x y = testY1_1 y x
  in testY2 x y

testY2 :: (F a, F b, G a, G b) => a -> b -> c
testY2 x y = testY1 y x

testZ1 :: (F a, G a) => a -> Bool
testZ1 x = testZ2 x x

testZ2 :: (G a, F a, G b) => a -> b -> Bool
testZ2 x y = testZ1 x
