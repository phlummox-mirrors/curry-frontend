...
genTypeDecl env (DataDecl _ _cx n vs cs)
  = ( resetScope env2
...
genTypeDecl env (NewtypeDecl _ _cx n vs (NewConstrDecl p nvs nc ty))
  = (resetScope env2
    , CType (genQName True env2 $ qualifyWith (moduleId env) n)
            (genVisibility env2 n)
            (zip idxs $ map idName vs)
...
buildExports _ [] = []
buildExports mid (DataDecl _ cx ident _ _:ds)
  = ExportTypeAll (qualifyWith mid ident) : buildExports mid ds
buildExports mid ((NewtypeDecl _ cx ident _ _):ds)
  = ExportTypeAll (qualifyWith mid ident) : buildExports mid ds
...
getConstrIdents (DataDecl _ _ _ _ cs) = map getConstr cs
...
isDataDeclOf i (DataDecl _ _ j _ _) = i == j
