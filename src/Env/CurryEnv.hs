...
-}
module Env.CurryEnv (CurryEnv (..), curryEnv) where
...
{- |Return a Curry environment for the module 'mod' and its corresponding
...
curryEnv _ tcEnv (Interface iid idecls) mod'@(Module mid mExp decls)
  | iid == mid = CurryEnv
      { moduleId     = mid
...
-- |
genImpIntf :: [(ModuleIdent, IDecl)] -> Decl -> [(ModuleIdent, IDecl)]
...
genTypeSynDeref its (TypeVariable i)
   = fromMaybe (internalError ("@CurryInfo.genTypeSynDeref: " ++
               ...
               (lookup i its)
genTypeSynDeref _ (TypeConstrained _ _)
...
genTypeSynDeref _ (TypeSkolem _)
   = internalError ("@CurryInfo.genTypeSynDeref: " ++
