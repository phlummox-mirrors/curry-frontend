...
> bindTypes m (DataDecl _ _ _ cs) env = foldr (bindConstr m) env cs
> bindTypes m (NewtypeDecl _ _ _ nc) env = bindNewConstr m nc env
> bindTypes m (TypeDecl _ t _ (RecordType fs _)) env =
...
>		   in  bindGlobal m
>	                          ident
>			          (GlobalVar (length ts) (qualifyWith m ident))
>	                          env
> bindFuncDecl m (ExternalDecl _ _ _ ident texpr) env
>    = bindGlobal m ident (GlobalVar (typeArity texpr) (qualifyWith m ident)) env
...
> bindVarDecl (PatternDecl _ t _) env
>    = foldr bindVar env (bv t)
> bindVarDecl (ExtraVariables _ vs) env
...
> checkDeclLhs _       k _ env (EvalAnnot p fs ev) =
...
> checkDeclLhs _       k _ env (ExternalDecl p cc ie f ty) =
>   return (ExternalDecl p cc ie (checkVar "external declaration" k env f) ty)
> checkDeclLhs _       k _ env (FlatExternalDecl p fs) =
...
> checkDeclLhs _       k _ env (ExtraVariables p vs) =
...
>              -> RenameState Decl
> checkDeclRhs _       bvs _ _ (TypeSig p vs ty) =
>   return (TypeSig p (map (checkLocalVar bvs ) vs) ty)
> checkDeclRhs _       bvs _ _ (EvalAnnot p vs ev) =
...
> checkConstrTerm _ _ _ _ _ (FunctionPattern _ _) = error $
...
> checkParenConstrTerm _ (ConstructorPattern _ constrTerms) =
