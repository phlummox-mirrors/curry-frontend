...
> ppDecl (FunctionDecl f vs ty expr) =
>   ppTypeSig f ty $$
>   sep [ppQIdent f <+> hsep (map ppIdent vs) <+> equals,
>        nest bodyIndent (ppExpr 0 expr)]
...
> ppBinding (Binding v expr) =
>   sep [ppIdent v <+> equals,nest bodyIndent (ppExpr 0 expr)]

> ppAlt :: Alt -> Doc
> ppAlt (Alt pat expr) =
>   sep [ppConstrTerm pat <+> text "->",nest altIndent (ppExpr 0 expr)]
